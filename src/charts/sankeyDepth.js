...
function buildDepthForLinkedNodes(sourceNodes, { childNodesByNode }) {
  ...
  function markNodeDepthAsMinimum(node, minimumDepth, ancestryNodes) {
    if (ancestryNodes.includes(node)) {
      return
    } // Stop the loop
    ...
    if (!childNodesByNode[node]) {
      return
    }
    childNodesByNode[node].forEach((childNode) =>
      markNodeDepthAsMinimum(childNode, minimumDepth + 1, [...ancestryNodes, node])
    )
  }
}
...
function assignMaximumDepthToRemainingNodes(depthByNode, nodes) {
  ...
  const nodesWithoutDepth = _.difference(
    nodes,
    Object.keys(depthByNode).map((key) => parseInt(key, 10))
  )
