...
export const getSankeyData = (actors, flows, { sankeyDisplayMode, monetaryCurrency }) => {
  ...
  result.series.levels = _.orderBy(_.uniq(Object.values(depthByActor))).map((_depth, index) => ({
    depth: index,
    lineStyle: {
      color: 'source',
      opacity: 0.4
...
function formatTooltip(titleKey, items) {
  ...
  const tooltipItems = listKeys.map((itemLabel) => formatTooltipItem(itemLabel, items[itemLabel]))
