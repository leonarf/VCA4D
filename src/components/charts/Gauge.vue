<template>
  <div class="flex flex-col w-4/5 lg:w-3/4 xl:w-1/2 ml-4 lg:ml-18 xl:ml-36 relative mt-8">
    <div class="bg-gradient-to-r from-green-300 via-yellow-200 to-red-300 w-full h-[40px] mt-10" />
    <div
      class="absolute top-20 transform -translate-x-1/2 -translate-y-full flex flex-col items-center"
    ...
    <div
      :class="`w-full pb-8 mb-4 ${hasBackground ? 'bg-gradient-to-r from-green-300/40 via-yellow-200/40 to-red-300/40' : ''}`"
    >
      ...
      <div class="w-full flex flex-row text-left">
        <div v-for="(text, index) in texts" :key="index" class="mt-2 px-2 w-full" v-html="text" />
      </div>
    </div>
  </div>
</template>
