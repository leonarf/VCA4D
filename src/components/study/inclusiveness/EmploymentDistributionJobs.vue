    ...
    <div class="flex flex-col mb-8">
      <div class="flex flex-row flex-wrap items-center justify-center">
        <div class="w-1/2 md:w-1/5 flex flex-col space-y-4 pt-8">
          <NiceMetric
            label="Waged employment"
            :description="fteDefinition"
            ...
            <template v-if="studyData">
              <template v-if="currentStageEmploymentByTypeOfActorData">
                <div class="w-1/3 aspect-w-1 aspect-h-1">
                  <Ring :options="currentStageEmploymentByTypeOfActorData" style="height: 300px" />
                </div>
              </template>
              <template v-else>
              ...
              <template v-if="currentStageEmploymentByGenderData">
                <div class="w-1/3 aspect-w-1 aspect-h-1">
                  <Ring
                    v-if="studyData"
                    :options="currentStageEmploymentByGenderData"
                    style="height: 300px"
                  />
                </div>
              </template>
...
const totalNumberOfJobs = computed(() => {
  return formatNumber(
    numberOfJobsData.value.series[0].data
...
const percentFemaleEmployment = computed(() => {
  if (!props.studyData.ecoData.consistencyDisplay.employment.genderRatio) {
    return null
  }
...
const fteDefinition = computed(() => {
  const definitionHtml =
    props.studyData?.ecoData?.macroData?.FTE_Definition || '<i>No FTE definition provided</i>'
  return `
...
const currentStageEmploymentByQualificationData = computed(() => {
  if (!props.studyData.ecoData.consistencyDisplay.employment.contractRatio) {
    return null
  }
  if (!selectedStageEmployment.value) {
    return null
  ...
  const { tempFemale, tempMale, unskilledFemale, unskilledMale, skilledFemale, skilledMale } =
    selectedStageEmployment.value
