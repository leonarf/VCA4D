...
async function fetchCachedStudyData(studyId) {
  if (cachedDataByStudyId[studyId]) {
    return cachedDataByStudyId[studyId]
  }
...
async function getStudyDataFromFileName(studyId, studyPart) {
  try {
    const result = await json(
      `${window.location.origin}${import.meta.env.DEV ? '/' : '/../VCA4D/'}data/${studyId}/${studyId}-${studyPart}.json`
    ).then((json) => json)
    return result
  } catch {
    return undefined
...
export function logMissingData(studiesData) {
  const studiesWithMissingData = studiesData.map(inventoryMissingData).filter(hasMissingData)
...
function formatReport(studiesWithMissingData) {
  if (studiesWithMissingData.length === 0) {
    return ''
  }

  return (
    `Studies are missing data:\n` +
    studiesWithMissingData
      .map(({ id, missingDataKeys }) => `- ${id} is missing: ${missingDataKeys.join(', ')}`)
      .join('\n')
  )
}
